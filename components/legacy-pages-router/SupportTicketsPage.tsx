...
const Card = ({ children }: { children: ReactNode }) => (
  <div className="border rounded p-4 shadow mb-4">{children}</div>
);
...
const DestructiveButton = ({
  ...
  <button className="px-4 py-2 bg-red-500 text-white rounded" {...props}>
    {children}
  </button>
);
...
const Textarea = (props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => (
  <textarea className="border p-2 rounded w-full" {...props} />
);
const Checkbox = ({
  checked,
  onChange,
...
export default function SupportTicketsPage() {
  ...
  useEffect(() => {
    try {
      ...
      ws.onmessage = (event) => {
        const incoming = JSON.parse(event.data) as MockTicket;
  ...
  const sendEmailNotification = (ticket: MockTicket) => {
