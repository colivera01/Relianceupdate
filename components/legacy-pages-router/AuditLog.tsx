          ...
          <div key={log.id} className={`border-b pb-4 cursor-pointer hover:bg-gray-50 rounded transition ${log.severity === 'critical' ? 'border-red-400 bg-red-50' : ''}`} onClick={() => setSelectedLog(log)}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <SimpleTooltip content={log.severity.charAt(0).toUpperCase() + log.severity.slice(1)}><Badge className={getSeverityColor(log.severity)}>{log.severity.toUpperCase()}</Badge></SimpleTooltip>
                <SimpleTooltip content={log.category.charAt(0).toUpperCase() + log.category.slice(1)}><Badge className={getCategoryColor(log.category)}>{log.category}</Badge></SimpleTooltip>
          ...
          {selectedLog ? (
            <>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div>
                  <b>Admin:</b> {selectedLog.adminName} ({selectedLog.adminId})
                </div>
                <div>
                  <b>Target:</b> {selectedLog.targetType} {selectedLog.targetId}
                </div>
                <div>
                  <b>Details:</b> {selectedLog.details}
                </div>
                <div>
                  <b>Timestamp:</b>{" "}
                ...
                <div>
                  <b>IP Address:</b> {selectedLog.ipAddress}
                </div>
                <div>
                  <b>Severity:</b> {selectedLog.severity}
                </div>
                <div>
                  <b>Category:</b> {selectedLog.category}
                </div>
                {selectedLog.changes && (
                  <div>
                    <b>Changes:</b>
