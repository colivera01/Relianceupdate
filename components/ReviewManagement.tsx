...
export default function ReviewManagement() {
  ...
  const updateStatus = (id: number, status: Status) => {
    ...
    setEditReason("");
    if (status === 'approved') {
      toast({ title: t.reviewApproved, description: `${t.reviewApproved} #${id}.`, variant: 'success' });
        ...
        {viewMode === 'grid' ? (
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {pagedReviews.map(r => {
              ...
              const getInitials = (str) => str.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
                ...
                <Card key={r.id} className="p-0 shadow-lg border rounded-xl overflow-hidden flex flex-col h-full">
                  ...
                  <div className="flex gap-2 px-4 pb-4 mt-auto">
                    {role === 'admin' && r.status === 'pending' && (
                      <>
                        <Button size="sm" variant="success" onClick={() => updateStatus(r.id, 'approved')}>{t.approve}</Button>
                ...
                <li key={r.id} className="p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center bg-white rounded shadow mb-2">
                  ...
                  {role === 'admin' && r.status === 'pending' && (
                    <div className="flex gap-2 mb-2 w-full sm:w-auto">
                      <Button size="xs" variant="success" onClick={() => updateStatus(r.id, 'approved')}>{t.approve}</Button>
                      <Button size="xs" variant="destructive" onClick={() => updateStatus(r.id, 'rejected')}>{t.reject}</Button>
                      <Button size="xs" variant="outline" onClick={() => logAction(r.id, 'flagged')}>{t.flag}</Button>
                    </div>
                  )}
                  {showCountdown && (
                  ...
                  <div className="flex justify-between items-center mb-2 w-full sm:w-auto">
                    <Tooltip content={r.auto ? t.autoModerated : t.manuallyReviewed}>
                      <Badge variant={r.auto ? 'destructive' : 'secondary'}>{r.auto ? t.auto : t.manual}</Badge>
                    </Tooltip>
                  ...
                  <CardContent className="w-full sm:w-auto">
                    ...
                    <div className="text-sm text-gray-700 mb-2">
                      {editingContent && viewing && viewing.id === r.id ? (
                        <>
                          ...
                          <Button size="xs" variant="success" className="mr-2" onClick={() => { updateStatus(r.id, r.status); logAction(r.id, 'edited', editReason); setEditingContent(false); }}>{t.save}</Button>
                          <Button size="xs" variant="outline" onClick={() => setEditingContent(false)}>{t.cancel}</Button>
                      ...
                      {role === 'admin' && (
                        <Button size="xs" variant="outline" className="ml-2" onClick={() => { setViewing(r); setContentInput(r.content); setEditingContent(true); }}>{t.edit}</Button>
                      )}
                    </div>
                    {/* Show image or video if present */}
                    {r.type === 'video' ? (
                      <video controls className="w-full rounded mb-2" loading="lazy"><source src={r.mediaUrl} type="video/mp4" /></video>
                    ) : (
                    ...
                    <p className="text-sm text-gray-600 truncate flex items-center gap-2 w-full sm:w-auto">
                      ...
                      <Button size="xs" variant="outline" className="ml-2" onClick={() => alert('Job/Service details coming soon!')}>{t.viewJob}</Button>
                    </p>
