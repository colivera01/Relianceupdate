...
| Historical migrations are not replayable from an empty database | `20250101000000_add_unique_active_assignment` references `DeviceAssignment` before that object exists | A clean disaster-recovery or onboarding database cannot run the full repository migration chain without a baseline | Create and validate a migration-baseline strategy without rewriting applied migration history | Private beta hardening | Open; does not block current beta additive migrations |
