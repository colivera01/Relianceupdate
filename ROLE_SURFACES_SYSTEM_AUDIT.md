...
| `/service/[serviceId]` | Service detail and booking entry | Partial | `GET /api/services/[id]`, `GET /api/services/[id]/reviews/public`, `GET /api/availability/vendor/[vendorId]`, favorites APIs | No in-app payment flow; availability edit pipeline still weak on vendor side | User sees vendor offering quality/media approved states | Dynamic (linked from discover/search) |
