...
export function buildVendorCoachingPlan(
  ...
  if (videoVerification && videoVerification.pct !== null && videoVerification.pct < 100) {
    priorityActions.push(
      `Video verification is ${videoVerification.pct}%. Tighten stage capture quality so Intro, In Progress, and Completed videos are clearly distinct and moderation-ready.`
    );
  }
