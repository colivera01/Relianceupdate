...
describe("private beta gate", () => {
  ...
  it("allows read-only physical inspection while acceptance mode is active", async () => {
  ...
  it("blocks business mutations while acceptance mode is active", async () => {
    process.env.RELIANCE_ACCEPTANCE_READ_ONLY = "YES";
    const request = new NextRequest("https://beta.relianceonline.org/api/bookings/example", {
      method: "PATCH",
    });
