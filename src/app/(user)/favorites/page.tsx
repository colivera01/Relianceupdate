...
export default function FavoritesPage() {
  ...
  if (!authLoading && !isAuthenticated) {
    return <main className="mx-auto max-w-4xl py-12"><h1 className="text-3xl font-semibold text-slate-950">Favorites</h1><p className="mt-3 text-slate-600">Sign in to see your saved services and businesses.</p><Link href="/auth/login?next=%2Ffavorites" className="mt-5 inline-flex rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white">Sign In</Link></main>;
  }
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 pb-12">
      <header className="border-b border-slate-200 pb-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-xs font-semibold uppercase text-blue-700">Repeat business</p><h1 className="mt-2 text-3xl font-semibold text-slate-950">Favorites</h1><p className="mt-2 text-sm text-slate-600">Save businesses you trust and public services you may want to use in the future.</p><p className="mt-1 text-sm text-slate-500">Your completed work remains in <Link href="/my-bookings" className="font-semibold text-blue-700 underline">My Service Records</Link>.</p></div>
        ...
        <label className="relative mt-5 block max-w-xl"><span className="sr-only">Search Favorites</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search saved services or businesses" className="w-full rounded-md border border-slate-300 py-2.5 pl-10 pr-3 text-sm" /></label>
      </header>
      ...
      <div role="tablist" aria-label="Favorite type" className="flex gap-2 border-b border-slate-200">
        {([
          ...
          ['vendor', 'Businesses', counts?.vendors ?? '--'],
      ...
      {authLoading || query.isPending || query.isFetching ? <p className="text-sm text-slate-600">Loading Favorites...</p> : query.isError ? <CustomerLoadError message="Unable to load Favorites." onRetry={() => void query.refetch()} /> : items.length === 0 ? <div className="py-12 text-center"><Heart className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 font-semibold text-slate-900">{empty}</p>{search ? <p className="mt-1 text-sm text-slate-500">Try a different search.</p> : filter === 'service' ? <p className="mt-1 text-sm text-slate-500">Save a public service from <Link href="/discover" className="font-semibold text-blue-700 underline">Explore Proof</Link> or a business&apos;s Services page.</p> : filter === 'vendor' ? <p className="mt-1 text-sm text-slate-500">Save a business you&apos;d like to use again.</p> : <p className="mt-1 text-sm text-slate-500">Explore businesses and services to save things you&apos;d like to revisit.</p>}</div> : (
...
function VendorCard({ item, busy, onRemove }: { item: Extract<FavoriteListItem, { entityType: 'vendor' }>; busy: boolean; onRemove: () => void }) {
  return <article className="rounded-md border border-slate-200 bg-white p-5"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-blue-50 text-blue-700"><Building2 className="h-5 w-5" /></span><div><p className="text-xs font-semibold uppercase text-blue-700">Saved Business</p><h2 className="text-lg font-semibold text-slate-950">{item.vendorName}</h2></div></div><RemoveButton busy={busy} onRemove={onRemove} /></div><div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-600">{item.location ? <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" />{item.location}</span> : null}<span>{item.serviceCount} available {item.serviceCount === 1 ? 'service' : 'services'}</span>{item.rating != null && item.reviewCount != null ? <span>{item.rating.toFixed(1)} Vendor Rating · {item.reviewCount}</span> : null}</div>{item.isPubliclyListed ? <div className="mt-5 flex gap-2"><Link href={`/vendors/${item.vendorId}?returnTo=${encodeURIComponent('/favorites')}&returnLabel=${encodeURIComponent('Back to Favorites')}`} className="rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white">View Business</Link></div> : <p className="mt-5 text-sm text-slate-600">This saved Business is not currently available in Explore Proof.</p>}</article>;
}

function ServiceCard({ item, busy, onRemove }: { item: Exclude<FavoriteListItem, { entityType: 'vendor' }>; busy: boolean; onRemove: () => void }) {
  const vendorEligible = item.publicListing.vendorEligible !== false;
  return <article className="rounded-md border border-slate-200 bg-white p-5"><div className="flex gap-4"><div className="h-24 w-32 shrink-0 overflow-hidden rounded-md bg-slate-100"><PublicMediaPreview url={item.previewMediaUrl} type={item.previewMediaType} alt={item.serviceName} className="h-full w-full object-cover" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-semibold uppercase text-blue-700">{item.publicListing.serviceEligible ? 'Saved Public Service' : 'Saved Service'}</p><h2 className="truncate text-lg font-semibold text-slate-950">{item.serviceName}</h2><p className="text-sm text-slate-600">Offered by {item.vendorName}</p></div><RemoveButton busy={busy} onRemove={onRemove} /></div>{item.rating != null && item.reviewCount != null ? <p className="mt-2 text-sm text-slate-600">{item.rating.toFixed(1)} Vendor Rating · {item.reviewCount}</p> : null}</div></div>{!item.publicListing.serviceEligible ? <p className="mt-5 text-sm text-slate-600">This saved Service is not currently available in Explore Proof.</p> : null}<div className="mt-5 flex flex-wrap gap-2">{item.publicListing.serviceEligible ? <Link href={`/service/${item.serviceId}?returnTo=${encodeURIComponent('/favorites')}&returnLabel=${encodeURIComponent('Back to Favorites')}`} className="rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white">View Service</Link> : null}{vendorEligible ? <Link href={`/vendors/${item.vendorId}`} className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700">View Business</Link> : null}{item.publicListing.serviceEligible ? <Link href={`/booking/${item.serviceId}`} className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700">Request Service</Link> : null}</div></article>;
}
