...
async function authorizeParticipant(userId: string, publication: any) {
  const { prisma } = await import("@/server/db");
  const membershipIds = await prisma.vendorMembership.findMany({
    where: { userId, vendorId: publication.proposal.vendorId, status: "ACTIVE", role: "EMPLOYEE" },
    select: { id: true },
  });
  const evidence = await prisma.serviceVideoStageEvidence.findMany({
    where: {
      id: { in: publication.stages.map((row: any) => row.stageEvidenceId) },
      employeeMembershipId: { in: membershipIds.map((row) => row.id) },
    },
    select: { id: true },
  });
...
export async function GET(request: Request, context: Context) {
  try {
    ...
    await authorizeParticipant(userId, publication);
