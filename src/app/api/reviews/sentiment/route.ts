...
import { assertReviewWindowActive, isValidSentiment } from '@/lib/review-capture';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reviewWindowId || !sentiment) {
      return NextResponse.json({ success: false, error: 'reviewWindowId and sentiment are required' }, { status: 400 });
    }
    if (!isValidSentiment(sentiment)) {
      return NextResponse.json({ success: false, error: 'Invalid sentiment value' }, { status: 400 });
    }
    ...
    const [record] = await Promise.all([
      ...
      (prisma as any).reviewPromptEvent.create({
        data: {
          reviewWindowId,
          eventType: sentiment === 'positive' ? 'clicked_positive' : sentiment === 'neutral' ? 'clicked_neutral' : 'clicked_negative',
    ...
    return NextResponse.json({ success: false, error: 'Failed to store sentiment' }, { status: 500 });
  }
}
