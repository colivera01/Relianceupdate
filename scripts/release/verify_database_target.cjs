...
async function main() {
  ...
  assert.equal(receipt.expectedDatabaseMigrationState?.targetSpecSha256, spec.sha256, 'Release receipt target specification differs');
  ...
  try {
    ...
    if (expected.structuralSha256) assert.equal(observed.structuralSha256, expected.structuralSha256, 'Structural fingerprint differs');
    if (expected.ledgerSha256) assert.equal(observed.ledgerSha256, expected.ledgerSha256, 'Migration ledger digest differs');
