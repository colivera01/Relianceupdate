...
const ACCEPTANCE_CHECKLIST_VERSION = 'reliance-cutover-v2-physical-v2';
...
function assertBinding(binding) {
  ...
  for (const field of ['runtimeArtifactSha256', 'runtimePackageReferenceSha256', 'migrationArtifactSha256',
    ...
    'deploymentControlSha256', 'durableFreezeControlSha256', 'parityManifestSha256']) {
  ...
  assert(binding.runtimePackageName?.endsWith('.zip'), 'Acceptance runtime package name is invalid');
  assert(Number.isSafeInteger(binding.runtimePackageSize) && binding.runtimePackageSize > 0,
    'Acceptance runtime package size is invalid');
...
function createAcceptanceReceipt({ output, state, decision, decisionSha256, authorizationReference,
  ...
  const receipt = {
    ...
    runtimePackageReferenceSha256: state.binding.runtimePackageReferenceSha256,
    runtimePackageName: state.binding.runtimePackageName,
    runtimePackageSize: state.binding.runtimePackageSize,
    ...
    durableFreezeControlSha256: state.binding.durableFreezeControlSha256,
    parityManifestSha256: state.binding.parityManifestSha256,
    health,
    smoke,
