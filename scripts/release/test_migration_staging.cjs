...
for (const stageName of ['baseline', 'reconciliation']) {
  ...
  try {
    ...
    if (stageName === 'baseline') {
      const unexpected = path.join(stage.stageRoot, 'prisma', 'migrations', '20260910030000_enforce_one_active_device_assignment');
...
try {
  fs.appendFileSync(path.join(corrupted.stageRoot, 'prisma', 'migrations', '00000000000000_reliance_forward_baseline_20260910', 'migration.sql'), '\n-- mutation');
